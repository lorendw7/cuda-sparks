...
Every preset and level here quietly stands on a real branch of math, physics, or CS —
you have been *using* numerical integration, vector calculus, chaos theory, stochastic
processes, procedural noise, and GPU architecture without formally studying them. That
full **study map** — seven theory domains, each tied to the exact kernel/line you wrote,
with a picture-first learn path (video → book) and a hands-on exercise, plus a priority
ladder — now lives in its own project-level document:

> **➜ [docs/MATH.md](../../docs/MATH.md) — The Mathematics Behind the Sim.**

It is optional and beyond the build; its job is to turn "it works" into "I understand
why." The two hands-on **capstone kernels** below are also specced in full there — both are
embarrassingly parallel, drop into this same GLFW/OpenGL + CUDA framework, and make
*fractal / bifurcation / attractor* tangible while doubling as clean CUDA practice:

- [ ] **Logistic-map bifurcation diagram** — iterate `x → r·x·(1 − x)`, one thread per
  `r`-column, plot the settled `x` values: the cleanest "order → chaos" picture (period
  doubling, chaos onset near `r ≈ 3.57`, the period-3 window, Feigenbaum's δ ≈ 4.669).
  Full spec in [docs/MATH.md](../../docs/MATH.md#logistic-map-bifurcation-diagram-do-this-first).
- [ ] **Mandelbrot / Julia set** — iterate `z → z² + c` per pixel, color by escape time,
  zoom into self-similar detail. One thread per pixel; the canonical fractal. Full spec in
  [docs/MATH.md](../../docs/MATH.md#mandelbrot--julia-set).
